...
class Crud_model extends CI_Model {
  ...
  function get_taluka($taluka_id = 0) {
    if ($taluka_id > 0) {
      $this->db->where('parent', $taluka_id);
    }
    $this->db->where('parent >', '0');
  ...
  function get_sub_categories_new($category_id = 0) {
    if ($category_id > 0) {
      $this->db->where('parent', $category_id);
    }
    $this->db->where('parent >', '0');
  ...
  function get_subcategory($category_id=0){
  if ($category_id > 0) {
      $this->db->where('id', $category_id);
    }
    return $this->db->get('category');
  ...
  function edit_subcategory($category_id = "") {
    $data['parent'] = 0;
    $data['name'] = sanitizer($this->input->post('name'));
...
//print_r($data);die;
  ...
  if ($data['categories_id'] == ''){
      ...
      $verified_categories = $this->db->get_where('listing', array('categories' => $data['categories_id']))->result_array();
      print_r($verified_categories);die;
      foreach($verified_categories as $verified_listing):
        $categories_id_array[] = $verified_listing['listing_id'];
      endforeach;
