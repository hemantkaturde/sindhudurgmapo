...
class Admin extends CI_Controller {
	...
	public function sub_categories($param1 = "", $param2 = "") {
		...
		if ($param1 == 'add') {
			$this->crud_model->add_subcategory();
			$this->session->set_flashdata('flash_message', get_phrase('sub_category_added'));
			redirect(site_url('admin/sub_categories'), 'refresh');
		}
		elseif ($param1 == 'edit') {
			$this->crud_model->edit_subcategory($param2);
			$this->session->set_flashdata('flash_message', get_phrase('sub_category_updated'));
			redirect(site_url('admin/sub_categories'), 'refresh');
		}elseif ($param1 == 'delete') {
			$this->crud_model->delete_from_table('category', $param2);
			$this->session->set_flashdata('flash_message', get_phrase('sub_category_deleted'));
			redirect(site_url('admin/sub_categories'), 'refresh');
		}
	...
	public function sub_categories_listing(){
		if ($this->session->userdata('admin_login') != true) {
			redirect(site_url('login'), 'refresh');
		}
		...
		$page_data['page_name'] = 'sub_categories';
		$page_data['page_title'] = get_phrase('sub_categories');
		$page_data['sub_categories'] = $this->crud_model->get_sub_categories_new()->result_array();
		$this->load->view('backend/index.php', $page_data);
	...
	public function sub_category_form($param1 = "", $param2 = "") {
		...
		if($param1 == 'add'){
			...
			$page_data['page_title'] = get_phrase('add_new_sub-category');
			$page_data['categories'] = $this->crud_model->get_categories()->result_array();
		}elseif ($param1 == 'edit') {
			$page_data['category_id'] = $param2;
	...
	function filter_listing_table() {
		...
		$page_data['categories_id'] 		= $_GET['categories_id'];
