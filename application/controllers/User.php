...
class User extends CI_Controller {
	...
	function payment_success($payment_method = "", $user_id = "", $package_id = "", $paid_amount = "", $session_id = "", $paymentToken = "", $payerID = "") {
		if ($this->session->userdata('user_login') != true) {
			redirect(site_url('login'), 'refresh');
		}

		$response = $this->crud_model->create_package_purchase_history($payment_method, $user_id, $package_id, $paid_amount, $session_id, $paymentToken, $payerID);
		if($response == true){
